...
// 更新小节表视频ID
   ...
   public function del(){
        ...
        $list = $Chapter->where('lesson_id='.$id)->getField('id',true);

        foreach($list as $key=>$val){

            $Section->where('zh_id='.$val)->delete();
    ...
    public function doedit(){
        ...
        $cate['id'] = $_POST['ed_cid'];
        $cate['zh_name'] = $_POST['ed_cna'];
        $Chapter->save($cate);

        $sate['id'] = $_POST['ed_sid'];
        $sate['xi_name'] = $_POST['ed_sca'];
        $Section->save($sate);
