...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/AP1/Public/css/interlocution.css" /> 

     <script type="text/javascript" src="/AP1/Public/js/jquery.cookie.js"></script>    

     <style type="text/css">
           
        #nav1 .current a{font-size:20px;color:black;}
        #nav2  {border-top:0px;}
        #nav2  li{padding-left:0px; border-bottom:0px;}
        #nav2  li a{height:40px;line-height:20px; }   
    </style>
</head>
<body>
    <div class="clear"></div>
...
<div class="clear"></div>
	...
	<div class="main  ">
		<div class="ww">
                        ...
                        <div class="wd-silder ">
                <div id="new1" style="margin-bottom:20px;display:none;" class=" new"><a href="<?php echo U('question');?>" class="newques">发现新问题</a></div>
                <div id="new2" style="margin-bottom:20px;display:inline;" class=" new"><a href="#0" class="newques">发现新问题</a></div>
                <script>
                ...
                <div class="clear"></div>
                <div style="display:inline;margin-top:20px;" class="wd-my">
                    <div class="clear"></div>
                    <div class="user-info">
                        <a href=""><img style="width:40px;height:40px;" id="id" class="img-circle myimg fl" src="/AP1/Uploads/<?php echo ($_SESSION['home']['img']); ?>"></a>
                        <p><a href=""><?php echo ($_SESSION['home']['name']); ?></a></p>
                    </div>
                     <script>
                        $a = '<?php echo ($_SESSION['home']['id']); ?>'; 
                    ...
                    <ul class="user-about">
                        <li><a href=""><b>提问</b></a></li>
                        <li><a href=""><b>回复</b></a></li>
                        <li><a href=""><b>关注</b></a></li>
                    </ul>
                </div>
                <script>
                    $a = '<?php echo ($_SESSION['home']['id']); ?>';
                  
                    if($a==''){$('.wd-my').css('display','none');}else{$('.wd-my').css('display','inline');}
                </script>
                <div class="clear"></div>
                <div  class="alltags clearfix"  >
                    <?php if(is_array($tlist)): foreach($tlist as $key=>$tval): ?><div><a class="per-tag " href="<?php echo U('tag','tid='.$tval['id']);?>"><?php echo ($tval["name"]); ?></a></div><?php endforeach; endif; ?>
                </div>
			...
			<div class="wd-main">
                ...
                <div class="tagarea">
                    <div class="tag-infos clearfix">
                        <img class="tagpic fl" src="/AP1/Uploads/<?php echo ($cdlist["tag-img"]); ?>" alt="" width="42" hight="42" />
                        <div class="qa-f-infos fl clearfix">
                            <h4 class="tag-name"><?php echo ($cdlist["name"]); ?></h4>
                            <span class="f-num">关注：<?php echo ($cdlist["care"]); ?>人</span>
                        </div>
                        <a  class="tag-will-fol at-tag" href="javascript:void(0)" data-tagids="2">关注</a>
                    </div>
                    <script>
                        
                        if(<?php echo ($carelist); ?>==1){
                           
                                $('.at-tag').css("color","red");
                                $('.at-tag').html("已关注");
                            }else{
                               
                                $('.at-tag').css("color","#CED1D1");
                                $('.at-tag').html("关注");
                            };
                        
                        $('.at-tag').click(function(){
                            if($('.at-tag').html().length==2){
                                $('.at-tag').css("color","red");
                                $('.at-tag').html("已关注");
                                $.get("<?php echo U(dotagcare);?>", { care_id: 1, cate_id:"<?php echo ($cdlist['id']); ?>"} );
                            }else{
                                $('.at-tag').css("color","#CED1D1");
                                $('.at-tag').html("关注");
                                $.get("<?php echo U(dotagcare);?>", { care_id: 0, cate_id:"<?php echo ($cdlist['id']); ?>"} );
                            };
                        });
                    ...
                    <p class="tag-introduce"><?php echo ($cdlist["content"]); ?></p>
                </div>
                
                 <div class="tag-nav">
                    <ul id="nav2" class="nav nav-pills">
                        <li role="presentation" class="active" ><a  href="<?php echo U('tag','cid=0&tid='.$tid);?>">最新</a></li>
                        <li role="presentation"  ><a  href="<?php echo U('tag','cid=1&tid='.$tid);?>">热门</a></li>
                        <li role="presentation"  ><a  href="<?php echo U('tag','cid=2&tid='.$tid);?>">等待回复</a></li>
			    	</ul>
                </div>

                <script>
                    
                    $("#nav2 li").eq($.cookie("tg_cid")).addClass('active').siblings().removeClass('active');
                </script>
        
            	            
			
                <div id="test" class="wd-list">
                    <?php if(!empty($list)): if(is_array($list)): foreach($list as $key=>$val): ?><div class="wenda-list">
                        
                        <div class="queshead  ">
                            <a href="">
                                <img style="width:40px;height:40px;" id="id" class="img-circle img" src="/AP1/Uploads/<?php echo ($val["user_pic"]); ?>">
                                <div><?php echo ($val["user_name"]); ?></div>
                            </a>
                        </div>

						<div class="wendamainlist clearfix ">
                             
                            <div class="ans-reply">
                                <a href="<?php echo U('detail','bw=1&id='.$val[id]);?>">

                                    <div class="ans-wen">
                                        <span><?php echo ($val["ans_count"]); ?></span>
                                        <div class="da">回答</div>
                                    </div>

                                    <div class="ans-liu">
                                        <span><?php echo ($val["ans_bw"]); ?></span>
                            ...
                            <div  class="wendatitle mb10 clearfix">
                                <small style="color:#0088CC;margin-right:10px;font-size:14px;"><i class="glyphicon glyphicon-question-sign"></i></small>   
                                <a href="<?php echo U('detail','bw=1&id='.$val[id]);?>" ><b><?php echo ($val["title"]); ?></b></a>
                            </div>
                            


                            <div class="wendare clearfix">
                                <small style="color:#EDF1F2;margin-right:10px;font-size:14px;"><i class="glyphicon glyphicon-comment"></i></small> 
                            <?php if($val['ans_count'] == 0){ ?>
								<span><?php echo ($val["ans_content"]); ?></span>
                            <?php }else{ ?>
                                <span>[<?php echo ($val["ans_type"]); ?>]</span>
                                <a href="<?php echo ($val["ans_user_id"]); ?>"><img style="width:16px;height:16px;" id="id" class="img-circle img" src="/AP1/Uploads/<?php echo ($val["ans_user_pic"]); ?>"></a>
                                <a href="<?php echo ($val["ans_user_id"]); ?>"><?php echo ($val["ans_user_name"]); ?>:</a>
                                <span><?php echo ($val["ans_content"]); ?></span>
                            <?php } ?>
                            ...
                            <div class="wdtime-tag">
                                <span class="wdtime">
                                    <small>提问时间:<?php echo ($val["time"]); ?></small>
                                </span>
                                <span class="wdtag">
                                	<?php if(is_array($val['tag'])): foreach($val['tag'] as $key=>$val1): ?><a class="wdtag-a " href="<?php echo U('tag','tid='.$val1['id']);?>"><?php echo ($val1["name"]); ?></a><?php endforeach; endif; ?>
                                </span>
                                <span class="wdsourse">
                                    <small><a href="<?php echo U('View/video','id='.$val['zz_id']);?>"><?php echo ($val["zname"]); ?></a></small>
                                </span>
                            </div>
