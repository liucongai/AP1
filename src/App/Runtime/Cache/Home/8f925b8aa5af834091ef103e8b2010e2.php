...
<link rel="stylesheet" type="text/css" href="/AP1/Public/css/saved_resource_pd2.css" />


<div class="plan-step-wrap js-raise-panel" id="js-raise-panel">
        <ul class="plan-step salary-raise-step clearfix">

            <?php if(is_array($detail)): foreach($detail as $key=>$val): ?><li class="step-item clearfix step-first">

            <i class="line"></i>
            <i class="dot"></i>
            <div class="step-content my_but">

                <h4> <?php echo ($val["name"]); ?></h4>
                <i hidden><?php echo ($val["id"]); ?></i>
                <p class="autowrap"><?php echo ($val["desc"]); ?></p>
            </div>
            <div class="step-medias-wrap" style="">
...
<script type="text/javascript">
    ...
    $('.my_but').find('h4').click(function(e){

        $('.step-medias-wrap').fadeOut('fast');
        $(this).parent().next().find('ul').children().remove('.course-one');
        $.get('<?php echo U("pl_ajaxName");?>',{id: $(this).next().text()},function(result){
            console.dir($(result));
            ...
            $.each(result,function(i,val){
                ...
                $('.my_but').next().find('ul').append('<li class="course-one">' +
